...
export default function PromotedPage() {
  ...
  const promotedItems = useMemo(() => {
    let items = allLiveItems.filter(item => item.isPromoted);
    if (searchTerm) {
    ...
    if (items.length === 0) {
        return (
            <div className="col-span-full text-center py-12 text-muted-foreground">
                <Star className="w-16 h-16 mx-auto text-primary/10 mb-4" />
                <h3 className="text-xl font-bold font-headline text-foreground">No Promoted Items Found</h3>
                <p>There are currently no promoted auctions. Check back later!</p>
            </div>
        )
    }
    return (
    ...
    <div className="container mx-auto px-4 py-12 md:py-16">
      ...
      <header className="mb-12">
        ...
        <p className="text-lg text-muted-foreground">
          Featured auctions from our sellers.
        </p>
      </header>
      ...
      <div className="relative w-full md:max-w-xs mb-8">
          ...
          <Input
              placeholder="Search promoted listings..."
      ...
      {renderAuctionGrid(promotedItems, areAllListingsLoading)}
    </div>
  );
}
