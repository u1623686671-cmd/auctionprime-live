...
import { Gavel, LogIn, Search, Users, X, Zap, ArrowLeft, Calendar, Star } from "lucide-react";
...
export default function UpcomingAuctionsPage() {
  ...
  const upcomingItems = useMemo(() => {
    ...
    return items;
  }, [allItems, searchTerm, getItemTitleSubtitle]);
    ...
    if (items.length === 0) {
        return (
             <div className="col-span-full text-center py-12 text-muted-foreground">
                <Calendar className="w-16 h-16 mx-auto text-primary/10 mb-4" />
                <h3 className="text-xl font-bold font-headline text-foreground">No Upcoming Auctions</h3>
    ...
    <div className="container mx-auto px-4 py-12 md:py-16">
      ...
      <header className="mb-12">
        ...
        <p className="text-lg text-muted-foreground">
          Get ready to bid on these items coming soon.
        </p>
      </header>
      ...
      {renderAuctionGrid(upcomingItems, areAllListingsLoading)}
    </div>
  );
}
