...
import { Home, LayoutGrid, Plus, User, LogOut, Gavel, DollarSign, Calendar, Loader2, Hand, Search, Wine, Gem, Palette, CreditCard, Phone, ChevronRight, Shirt, Zap, X, LogIn, Star, ShoppingBag } from "lucide-react";
import {
  Tabs,
  TabsContent,
  TabsList,
...
export default function HomePage() {
  ...
  const auctionCategories = useMemo(() => [
    { title: 'Alcohol', icon: Wine, data: allAlcohol, type: 'alcohol' },
    { title: 'Art', icon: Palette, data: allArt, type: 'art' },
    { title: 'Apparel', icon: Shirt, data: allApparels, type: 'apparels' },
    { title: 'Iconic Items', icon: Gem, data: allIconics, type: 'iconics' },
    { title: 'Other', icon: ShoppingBag, data: allOthers, type: 'others' },
    { title: 'Car Plates', icon: CreditCard, data: allPlates, type: 'plates' },
    { title: 'Phone Numbers', icon: Phone, data: allPhoneNumbers, type: 'phoneNumbers' },
  ...
  const allItems = useMemo(() => {
    ...
    return items.sort((a, b) => {
        ...
        if (!a.isPromoted && b.isPromoted) return 1;
        const aIsLive = new Date(a.auctionStartDate).getTime() <= now.getTime() && new Date(a.auctionEndDate).getTime() > now.getTime();
        const bIsLive = new Date(b.auctionStartDate).getTime() <= now.getTime() && new Date(b.auctionEndDate).getTime() > now.getTime();
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        {auctionCategories.map(cat => (
                            ...
                            >
                                ...
                                {cat.title === 'Iconic Items' ? 'Iconic' : cat.title}
                            </Button>
                        ))}
                    </div>
